...
# 3️⃣ ARCHITECTURE DIAGRAM PROMPT

Use this prompt in any diagramming tool (draw.io, Lucidchart, Excalidraw, etc.):
```
Create a detailed AWS disaster recovery architecture diagram with the following components:

TITLE: "Vendor Sales & Finance Analytics Dashboard - DR Architecture"

LEFT SIDE - Primary Region (Mumbai ap-south-1):
- Region box labeled "Primary: Mumbai (ap-south-1)"
- API Gateway icon labeled "vendor-API (0wek2322jl)"
- Four Lambda function icons:
  * Transaction_Lambda
  * Orders_Lambda
  * Dashboard_Data_Lambda
  * analytics-alerts
- DynamoDB table icon labeled "VendorSales (Global Table)"
- Cognito icon labeled "User Pool"
- All contained within a blue-bordered region box

RIGHT SIDE - DR Region (Singapore ap-southeast-1):
- Region box labeled "DR: Singapore (ap-southeast-1)"
- API Gateway icon labeled "vendor-API (d9wo0hh9q9)"
- Same four Lambda functions (replicated)
- DynamoDB table icon labeled "VendorSales (Replica)"
- Note: "Uses Mumbai Cognito"
- All contained within a green-bordered region box

TOP CENTER - Route 53:
- DNS icon labeled "Route 53"
- Domain: "api.vendorsales.company.com"
- Health Check icon with ID "6b0190c7-f241-466e-b939-a1cf0768f4f0"
- Failover policy indicator
- Arrows pointing to both regions

CONNECTIONS:
- Solid blue arrow from Route 53 to Mumbai (labeled "Primary - Active")
- Dashed green arrow from Route 53 to Singapore (labeled "Secondary - Standby")
- Bidirectional dotted arrow between Mumbai DynamoDB and Singapore DynamoDB (labeled "Replication ~1.1s")
- Arrow from Mumbai Cognito to Singapore API (labeled "JWT Validation")

BOTTOM - Metrics Box:
- RPO: 1.09 seconds
- RTO: 5 seconds (direct) / 4 minutes (with DNS)
- Data Consistency: 100%
- Availability: 99.99%

Color scheme:
- Primary region: Blue theme
- DR region: Green theme
- Active connections: Solid lines
- Failover connections: Dashed lines
- Replication: Dotted lines
